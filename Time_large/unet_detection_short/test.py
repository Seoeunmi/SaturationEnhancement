...
with multi_gpu_strategy.scope():
    ...
    frame_size_detection = past_size_detection + present_size_detection + future_size_detection
    ...
    def test_step(dist_inputs):
        ...
        def step_fn(inputs):
            ...
            x = tf.pad(x, [[0, 0], [shift_size_detection, shift_size_detection]])
            y_pred = tf.pad(y_pred, [[0, 0], [shift_size_detection, shift_size_detection]])
            y_true = tf.pad(y_true, [[0, 0], [shift_size_detection, shift_size_detection]])

            number_of_frame = (x.shape[1] - frame_size_detection + shift_size_detection) // shift_size_detection
            out = tf.zeros([x.shape[0], shift_size_detection + frame_size + shift_size_detection])
            # window = util.custom_hann(frame_size_detection)
            window = util.sine_window(frame_size_detection)
            for i in range(number_of_frame):
                x_slice = tf.slice(x, [0, i * shift_size_detection], [x.shape[0], frame_size_detection]) * window
                y_pred_slice = tf.slice(y_pred, [0, i * shift_size_detection], [y_pred.shape[0], frame_size_detection]) * window
                y_true_slice = tf.slice(y_true, [0, i * shift_size_detection], [y_true.shape[0], frame_size_detection]) * window

                labels = tf.where(tf.reduce_sum(tf.abs(y_true_slice - x_slice), axis=1) > 1e-5, 1., 0.)

                predictions = tf.round(detection(tf.expand_dims(x_slice, 2), tf.expand_dims(y_pred_slice, 2)))
                out_silce = (predictions * y_pred_slice + (1 - predictions) * x_slice) * window
                out_silce = tf.pad(out_silce, [[0, 0], [i * shift_size_detection, out.shape[1] - (i * shift_size_detection + frame_size_detection)]])
                # out = tf.add(out, out_silce)
                out += out_silce
            out = tf.slice(out, [0, shift_size_detection], [out.shape[0], frame_size])
