...
with multi_gpu_strategy.scope():
    ...
    def train_step(dist_inputs):
        ...
        def step_fn(inputs):
            ...
            with tf.GradientTape(persistent=False) as tape:
                predictions = tf.squeeze(detection_before(tf.expand_dims(x, 2)), axis=1)
                loss = bce(labels, predictions)

            gradients = tape.gradient(loss, detection_before.trainable_variables)
            optimizer.apply_gradients(zip(gradients, detection_before.trainable_variables))
    ...
    for epoch in range(saved_epoch + 1, saved_epoch + epochs + 1):
        ...
        if epoch % save_checkpoint_period == 0 or epoch == 1:
            ...
            detection_before.save_weights(os.path.join(save_checkpoint_path, 'detection_data.ckpt'))
