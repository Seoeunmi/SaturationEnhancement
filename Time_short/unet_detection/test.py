...
with multi_gpu_strategy.scope():
    ...
    def test_step(dist_inputs):
        ...
        def step_fn(inputs):
            ...
            if local_bath_size != 0:
                ...
                predictions_list = tf.split(tf.ones_like(x) * predictions, num_or_size_splits=local_bath_size, axis=0)
                for i in range(local_bath_size):
                    return_list.append([index[i], tf.squeeze(out_list[i]), tf.squeeze(x_list[i]), predictions_list[i]])
    ...
    for i, (source_file, target_file) in enumerate(zip(source_file_list, target_file_list)):
        ...
        for index, orig_pred, orig, prediction in return_list:
            index = int(index)
            orig_pred = orig_pred.numpy()
            orig_pred_output[index*shift_size+past_size:index*shift_size+past_size+frame_size] += orig_pred * util.custom_kbd(frame_size)
            # orig_output[index*shift_size+past_size:index*shift_size+past_size+frame_size] += orig * util.window('sine', present_size)
            # pred_output[index*shift_size+past_size:index*shift_size+past_size+frame_size] += 0.5 * prediction * util.window('hanning', present_size)
